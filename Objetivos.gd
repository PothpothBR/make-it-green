...
func _ready():
	...
	area_limpa = "Limpeza da Área: 0/10"
	arvores_plantadas = "Árvores Plantadas: 0/10"
...
func limpar(lixoAtual: int) -> void:
	var limpeza: int = lixoTotal - lixoAtual
	if limpeza < 10:
...
func plantar(lixoAtual: int) -> void:
	var arv_plan: int = lixoTotal - lixoAtual
	if arv_plan < 10:
